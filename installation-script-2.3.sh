...
# CONFIGURATION INPUT
# ============================================================
step "[STEP 2/12] CONFIGURATION INPUT"
...
# Use timeout (GNU coreutils) if available to prevent dns lookup hanging indefinitely
IP_PUBLIC=""
...
# Fallback to local IP if public IP could not be retrieved
if [[ -z "$IP_SUFFIX" ]]; then
    info "Failed to get public IP, attempting to use local IP..."
...
step "[STEP 4/12] SYSTEM PREPARATION — APT UPDATE & UPGRADE"
...
log "Running apt-get upgrade (non-interactive, needrestart suppressed)..."
...
# DOCKER
# ============================================================
step "[STEP 5/12] DOCKER INSTALLATION"
...
if ! command -v docker &>/dev/null; then
    ...
    if command -v curl &>/dev/null; then
        ...
        warn "curl and wget not found. Attempting to install curl..."
...
if sudo ss -tlnp | grep -q ":${NEW_SSH_PORT}[[:space:]]"; then
    ...
    warn "SSH does not appear to be listening on port ${NEW_SSH_PORT} — verify connectivity before continuing."
fi

# ============================================================
# PULL IMAGES
# ============================================================
step "[STEP 7/12] PULLING HONEYPOT DOCKER IMAGES"
...
# VOLUMES
# ============================================================
step "[STEP 8/12] CREATING DOCKER VOLUMES"
...
# ============================================================
step "[STEP 9/12] STARTING HONEYPOT CONTAINERS"
...
# NATS LEAF NODE
# ============================================================
step "[STEP 10/12] NATS LEAF NODE SETUP"
...
# FLUENT BIT
# ============================================================
step "[STEP 11/12] FLUENT BIT SETUP"
...
# ZABBIX AGENT
# ============================================================
step "[STEP 12/12] ZABBIX AGENT INSTALLATION"
